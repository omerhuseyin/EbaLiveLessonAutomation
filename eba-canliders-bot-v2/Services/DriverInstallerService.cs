...
namespace eba_canliders_bot_v2.Forms
{
    public partial class DriverInstallerService : Form
    {
        ...
        int seconds = 0;
        const string _chromePath = @"chromedriver.exe";
        const string rarPath = @"driver.rar";
        const string zipPath = @"driver.rar";
...
.Select(li => li.InnerText).ToList();
        ...
        {
            Services.DiscordEventListener.Send("Starting EbaBot Client...");
            try
            {
        ...
        private void RedirectController_Tick(object sender, EventArgs e)
        {
            ...
            if (seconds == 3)
            {
                ServiceStarter redirectMain = new ServiceStarter();          
                this.Hide();      
                redirectMain.Show();
            }
        }
